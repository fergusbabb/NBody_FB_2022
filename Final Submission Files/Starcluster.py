...
N2 = N - N1
lim = 20 #AU
quality = 500 #higher is better, but too high leads to very slow animation
off = AU/1e2 #relatively small number to avoid division by 0

ti = 0
tf = year
steps = int(N*10000*tf/year) #too small -> singularity, too large -> too slow,
#needs to be proportional to total time and number of particles
times = np.linspace(ti, tf, steps)
BH1mass = Msun*1e3
BH2mass = Msun*2e3

#_____________Code for 2 clusters with N1, N2 bodies respectively______________
...
#Mass distribution
def f(x):
    return (x**-1.05) #standard is x^-1.2 ish
...
locx1, locy1 = 15*AU, 8*AU

sigx2, sigy2 = AU*2.5, AU*2.5
locx2, locy2 = -15*AU, -8*AU
...
'''
#Define normal orbital velocities for two bodies of masses of each cluster
#orbiting in a certain time period at some distance from each other
...
#Define angle of attack of BH and adjust bodies accordingly
...
sol = odeint(ode, init, times, args = (masses, G, off, size, d), rtol = rtol)
x = sol[:,0:d]
y = sol[:,d:2*d]
vx = sol[:,2*d:3*d].T
vy = sol[:,3*d:4*d].T
...
#Plot initial points as crosses
for i in range(0,N):
...
end_time = time.time()
print('Elapsed time for N = {} was'.format(N), repr(end_time-start_time), 's')

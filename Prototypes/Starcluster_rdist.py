...
x0 = vals[1, 0:N]
y0 = vals[2, 0:N]  # Distances to center of mass, not from center of earth
vx0 = vals[3, 0:N]
vy0 = vals[4, 0:N]
...
s = abs(1.5+3*np.log10(Msun/masses) + np.min(np.log10(Msun/masses)))
...
xposax.set(xlim = (ti, tf), ylim = (-2*lim, 2*lim),
           #change min to lim/20 for r vs t plot or -2lim for x vs t plot
           xticks = [tf/4, tf/2, 3*tf/4, tf],
           xticklabels = ['${:.2f}$'.format(tf/(4*year)),
                          '${:.2f}$'.format(tf/(2*year)),
                          '${:.2f}$'.format(3*tf/(4*year)),
                          '${:.2f}$'.format(tf/year)],
           yticks = ticks2, yticklabels = labels2, 
           xlabel = '$t$ [yrs]', ylabel = '$r$ [AU]')

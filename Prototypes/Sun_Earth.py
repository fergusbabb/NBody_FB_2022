...
AU = 384400000  # m - earth moon AU, not standard AU!
Rearth = 6.371e6
day = 60*60*24  # s
year = day*365.25  # s
...
masses = vals[0, idx]
x0 = vals[0, idx]
y0 = vals[0, idx]  # Distances to center of mass, not from center of earth
vx0 = vals[0, idx]
vy0 = vals[0, idx]
#vy0[0] = vy0[1]*x0[0]/x0[1] #velocity of earth arount earth-moon com
#masses[-1]*=10
...
'''
X = x.copy() # need the copies to avoid only chaning one set of data and then
#minusing 0 from everything else
Y = y.copy()
for i in range(0, N):
    x[:, i] -= X[:, 0]
    y[:, i] -= Y[:, 0]
...
zmin = -G*masses[0]/(Rearth*10)  # Potential at r = 8*Rearth
...
surfax.contour(xt/AU, yt/AU, -np.log(Usurf**2), levels=30, cmap=cm.cool)
...
sizes = [5, 2]
clrs = ['greenyellow', 'whitesmoke']

surfax.plot(x[0,0], y[0,0], 'o', c=clrs[0], markersize='10')
surfax.plot(x[0,-1], y[0,-1], 'o', c=clrs[1], markersize='8')
...
surfax.plot(1- L1(x0[0], masses[0], masses[-1])/AU, 0, 'r+')
print(1 - L1(x0[-1]-x0[0], masses[0], masses[-1])/AU)

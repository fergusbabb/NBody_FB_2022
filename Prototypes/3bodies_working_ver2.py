...
def acc(pos, vel, M):#, acc):
    ...
    r2 = dx**2 + dy**2 + dz**2 + off**2
    r3in = r2**-1.5

    ax = (dx * r3in) @ M
    ay = (dy * r3in) @ M
    az = (dz * r3in) @ M
    a = G*np.vstack((ax,ay,az)).T 
    #Not entirely sure about .T but it corrected the issue I had
    
    rin = r2**-.5
    
    print(np.shape(ax))
    mM = Mvec*Mvec.T
    mM -= np.diag(np.diag(mM))
    mM_rin = np.sum(np.sum(mM)*rin)
